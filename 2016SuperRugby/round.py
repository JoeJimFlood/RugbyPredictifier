...
matchups['Friday'] = [('HURRICANES', 'KINGS')]
matchups['Saturday'] = [('CHIEFS', 'FORCE'),
                        ('REBELS', 'HIGHLANDERS'),
                        ('SUNWOLVES', 'BULLS'),
                        ('CHEETAHS', 'BRUMBIES'),
                        ('SHARKS', 'CRUSADERS'),
                        ('JAGUARES', 'STORMERS')]
matchups['Sunday'] = [('REDS', 'WARATAHS')]
...
for read_data in range(1):
    ...
    for game_time in matchups:
        ...
        for i in range(len(games)):
            ...
            values = [hwin, awin, draw]
            colors = [colours[home][0], colours[away][0], '#777777']
            ex = 0.05
            explode = [ex, ex, ex]
            plt.pie(values,
                    colors = colors,
