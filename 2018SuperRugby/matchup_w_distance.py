...
def get_expected_scores(team_1_stats, team_2_stats, team_1_df, team_2_df): #Get the expected scores for a matchup based on the previous teams' performances
    ...
    for stat in team_1_stats:
        expected_scores.update({'T': mean([team_1_stats['TF'] + np.average(team_2_df['TA'], team_2_df['Weight']),
                                           team_2_stats['TA'] + np.average(team_1_df['TF'], team_1_df['Weight'])])})
        expected_scores.update({'P': mean([team_1_stats['PF'] + np.average(team_2_df['PA'], team_2_df['Weight']),
                                           team_2_stats['PA'] + np.average(team_1_df['PF'], team_1_df['Weight'])])})
        expected_scores.update({'DG': mean([team_1_stats['DGF'] + np.average(team_2_df['DGA'], team_2_df['Weight']),
                                            team_2_stats['DGA'] + np.average(team_1_df['DGF'], team_1_df['Weight'])])})
...
def matchup(team_1, team_2, venue = None):
    ...
    team_1_season['Weights'] = team_1_season['VENUE'].apply(get_team_1_weight)
    team_2_season['Weights'] = team_2_season['VENUE'].apply(get_team_2_weight)
