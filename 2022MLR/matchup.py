...
def matchup(team_1, team_2, venue = None):
    ...
    var_1 = pd.Series(0.25*(variances_1.reindex(['TF', 'PF', 'DF']).values + variances_2.reindex(['TA', 'PA', 'DGA']).values), ['T', 'P', 'DG'])
    var_2 = pd.Series(0.25*(variances_2.reindex(['TF', 'PF', 'DF']).values + variances_1.reindex(['TA', 'PA', 'DGA']).values), ['T', 'P', 'DG'])
